          ...
          <div class="basket-item__info">
            <div class="basket-item__head">
              <h2 class="basket-item__name">
                {{ \App\Models\Category::find($product->category_id)['name_' . session()->get('lan')] }}
                ({{ $product->size }}L)
              </h2>
            ...
            <div class="basket-item__open @if ($cart[$key]['display'] == 'block') active @endif">
              количество
              <svg width="15" height="9" viewBox="0 0 15 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M14 1L7.5 7L1 1" stroke="#217BBE" stroke-width="1.5" stroke-linecap="round" />
              </svg>
            </div>
            <div class="basket-item__dropdown" style="display: {{ $cart[$key]['display'] }}">
                ...
                <div class="order-item order-print">
                  ...
                  <div class="order-print__btns">
                    <button class="btn @if ($cart[$key]['logo'] == 1) active @endif"
                      onclick="logo(1, {{ $product->id }})"
                      value="1">{{ __('order.С вашим логотипом') }}</button>
                    <button class="btn @if ($cart[$key]['logo'] == 0) active @endif"
                      onclick="logo(0, {{ $product->id }})" value="0">{{ __('order.Без логотипа') }}</button>
                  </div>
  ...
  <div class="basket-content" id="basket-content">
    <form action="{{ Route('order.create', session()->get('locale')) }}" class="basket-form">
      ...
      <div class="form-half">
        ...
        <input name="phone" type="tel" placeholder="{{ __('order.homeContactPhone') }}" class="form__tel"
      ...
      <div class="order-item order-delivery">
        ...
        <div class="order-delivery__btns">
          <button name="deliveryNeeded" class="btn active"
            type="button">{{ __('order.cartFormDelivery1') }}</button>
      ...
      <div class="basket-form__choose">
        <div>
          <input type="radio" name="pay" id="cash"> <label
            for="cash">{{ __('order.cartFormPaymentM1') }}</label>
        </div>
        <div>
          <input type="radio" name="pay" id="card"> <label
            for="card">{{ __('order.cartFormPaymentM2') }}</label>
        </div>
      </div>
